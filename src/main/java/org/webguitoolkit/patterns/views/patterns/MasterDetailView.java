...
public class MasterDetailView extends AbstractView {
	...
	public MasterDetailView(WebGuiFactory factory, ICanvas viewConnector) {
	...
	protected void createControls( WebGuiFactory factory, ICanvas viewConnector ) {
		...
		//create Master Table
		table = factory.createTable(layout, "", 6);
		...
		MasterDetailListener masterDetailListener = new MasterDetailListener(table, tabstrip);
		table.setListener(masterDetailListener);
		tabstrip.setListener(masterDetailListener);

		summaryTab = factory.createTab(tabstrip, "Summary");

		detailTab = factory.createTab(tabstrip, "Details");

		summaryCompound = factory.createCompound(summaryTab);
		...
		detailCompound = factory.createCompound(detailTab);
	 ...
	 * 
	 */
	class MasterDetailListener extends AbstractMasterDetailListener {
		...
		public void loadTab(IDataBag row, ITab tab) {
			if (tab == summaryTab) {
				//reset Bag
				if (summaryCompound.getMode() == Compound.MODE_NEW || summaryCompound.getMode() == Compound.MODE_EDIT) {
					summaryCompound.getBag().undo(); // undo changed within bag
					summaryCompound.changeElementMode( Compound.MODE_READONLY );
				}
				summaryCompound.setBag(row);
				summaryCompound.load();
			}
			if (tab == detailTab) {
				//reset Bag
				if (detailCompound.getMode() == Compound.MODE_NEW || detailCompound.getMode() == Compound.MODE_EDIT) {
					detailCompound.getBag().undo(); // undo changed within bag
					detailCompound.changeElementMode( Compound.MODE_READONLY );
				}
				detailCompound.setBag(row);
				detailCompound.load();
			}
		}
		...
		public boolean leaveTab(IDataBag row, ITab tab) {
			if (tab == summaryTab) {
				if (summaryCompound.getMode() == Compound.MODE_NEW || summaryCompound.getMode() == Compound.MODE_EDIT) {
					tab.getPage().sendWarn("please save your changes before leaving the tab");
					return false;
				}
			}
			if (tab == detailTab) {
